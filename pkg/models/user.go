...
func (u *User) CommitteeByID(id int64) *Committee {
	if ms := u.MembershipByID(id); ms != nil {
		return ms.Committee
	}
	return nil
}
...
func DeactivateUsersByNickname(
	...
	for nickname := range nicknames {
		...
		UpdateMembershipsTx(ctx, tx, nickname, maps.Values(memberships), nil, &decisionMaker)
...
func UpdateMemberships(
	...
	defer tx.Rollback()
	err = UpdateMembershipsTx(ctx, tx, nickname, memberships, meetingID, decisionMaker)
	if err != nil {
		return err
	}
	return tx.Commit()
...
func UpdateUserCommitteeStatusTx(
	...
	for nickname, status := range users {
		...
		if _, err := iStmt.ExecContext(
			ctx, nickname, committeeID, status, since, pending, meetingID, decisionMaker); err != nil {
